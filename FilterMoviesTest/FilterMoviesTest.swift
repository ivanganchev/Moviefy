...
class FilterMoviesTest: XCTestCase {
    ...
    func testFilterWithOneGenre() throws {
        let filteredMovies = FilterHelper.getMoviesByGenre(movies: movies, selectedGenres: ["Horror"], allGenres: allGenres)
        
        XCTAssertEqual(2, filteredMovies.count)
    }
    
    func testFilterWithMultipleGenres() throws {
        let filteredMovies = FilterHelper.getMoviesByGenre(movies: movies, selectedGenres: ["Western", "Action", "Adventure"], allGenres: allGenres)
