...
class TransitionAnimator: NSObject, UIViewControllerAnimatedTransitioning {
    static let duration: TimeInterval = 0.5
        ...
        private let categoryCollectionViewController: CategoryCollectionViewViewController
        private let movieInfoViewController: MovieInfoViewController
        private var selectedCellImageViewSnapshot: UIView
        private let cellImageViewRect: CGRect

        init?(type: PresentationType, categoryCollectionViewController: CategoryCollectionViewViewController, movieInfoViewController: MovieInfoViewController, selectedCellImageViewSnapshot: UIView) {
            self.type = type
            self.categoryCollectionViewController = categoryCollectionViewController
            self.movieInfoViewController = movieInfoViewController
            self.selectedCellImageViewSnapshot = selectedCellImageViewSnapshot

            guard let window = categoryCollectionViewController.view.window ?? movieInfoViewController.view.window,
                let selectedCell = categoryCollectionViewController.selectedCell
                else { return nil }
        ...
        func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
            ...
            if isPresenting {
                ...
                backgroundView = categoryCollectionViewController.view.snapshotView(afterScreenUpdates: true) ?? fadeView
                backgroundView.addSubview(fadeView)
            }
